...
async function seedDB() {
    ...
    try {
        ...
        try {
            await database.createCollection("transaction_history");

        } catch (error) {
            console.log("Collection is available");
        }

        const collection = await database.collection("transaction_history");
        ...
        for (let y = 0; y < 100; y++) {
            ...
            for (let i = 0; i < 1000; i++) {
                ...
                timeSeriesData.push(transactionHistory());
